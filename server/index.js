...
if (!state.config) { state.config = readJSON(defaultConfigPath, null); writeJSON(dbPath, state); }
...
app.post('/api/config', (req, res) => {
  ...
  state.config = cfg; writeJSON(dbPath, state);
...
app.post('/api/opportunities', (req, res) => {
  ...
  const required = (state.config?.fields?.global || []).filter(f => f.required);
  ...
  state.opportunities.push(opp); writeJSON(dbPath, state);
  res.status(201).json(opp);
});
app.patch('/api/opportunities/:id/move', (req, res) => {
  const { id } = req.params; const { to_stage } = req.body || {};
  const stageIds = (state.config?.stages || []).map(s => s.id);
  if (!stageIds.includes(to_stage)) return res.status(400).json({ error: 'Etapa destino inválida' });
  const idx = state.opportunities.findIndex(o => o.id === id);
  if (idx === -1) return res.status(404).json({ error: 'No encontrada' });
  state.opportunities[idx].stage = to_stage; state.opportunities[idx].moved_at = new Date().toISOString();
  writeJSON(dbPath, state); res.json(state.opportunities[idx]);
});
...
app.listen(port, () => console.log(`AutoFlow Demo en http://localhost:${port}`));
